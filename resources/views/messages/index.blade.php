        ...
        @if(request()->route()->named('sendportal.messages.index'))
            <div class="mr-2">
                <select name="status" class="form-control">
                    ...
                    <option value="delivered" {{ request('status') == 'delivered' ? 'selected' : '' }}>{{ __('Delivered') }}
                    </option>
                    ...
                    <tr>
                        ...
                        @if(request()->route()->named('sendportal.messages.draft'))
                            ...
                            <th>
                                ...
                                <form action="{{ route('sendportal.messages.send-selected') }}" method="post"
                                    id="send-selected-form" style="display: inline-block;">
                                    @csrf
                ...
                <tbody>
                    @forelse($messages as $message)
                        <tr>
                            ...
                            <td>
                                @if($message->isCampaign())
                                    ...
                                    <a href="{{ route('sendportal.campaigns.reports.index', $message->source_id) }}">
                                        {{ $message->source ? $message->source->name : __('Campaign') }}
                                    </a>
                                @elseif($message->isAutomation())
                                    ...
                                    @if($message->source && $message->source->automation_step && $message->source->automation_step->automation)
                                        <a
                                            href="{{ route('sendportal.automations.show', $message->source->automation_step->automation_id) }}">
                            ...
                            <td><a
                                    href="{{ route('sendportal.subscribers.show', $message->subscriber_id) }}">{{ $message->recipient_email }}</a>
                            </td>
                            <td>
                            ...
                            @if(request()->route()->named('sendportal.messages.draft') && !$message->sent_at)
                                <td>
                                    ...
                                    <form action="{{ route('sendportal.messages.delete', $message->id) }}" method="post"
                                        class="d-inline-block delete-message">
                                        @csrf
                                        @method('delete')
                                ...
                                <td>
                                    <input type="checkbox" name="messages[]" value="{{ $message->id }}" class="message-select"
